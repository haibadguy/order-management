...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        try {
            $stmt = mysqli_prepare($conn, 'SELECT COUNT(*) as total FROM customer WHERE email = ?');
    ...
    // Nếu không có lỗi thì thêm khách hàng mới
    if (empty($errors)) {
        try {
            $stmt = mysqli_prepare($conn, 'INSERT INTO customer (full_name, email, phone, avatar, status) VALUES (?, ?, ?, ?, ?)');
            mysqli_stmt_bind_param($stmt, 'sssss', $full_name, $email, $phone, $avatar, $status);
            mysqli_stmt_execute($stmt);
            ...
            $errors['db'] = 'Lỗi khi thêm khách hàng: ' . $e->getMessage();
        }
    }
}
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select <?php echo isset($errors['status']) ? 'is-invalid' : ''; ?>" 
                        ...
                        <option value="blocked" <?php echo (isset($_POST['status']) && $_POST['status'] === 'blocked') ? 'selected' : ''; ?>>Đã khóa</option>
                    </select>
