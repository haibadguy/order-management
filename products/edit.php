...
try {
    ...
    setMessage('Lỗi khi lấy thông tin sản phẩm: ' . $e->getMessage(), 'danger');
    header('Location: index.php');
    exit();
}
        ...
        try {
            $stmt = mysqli_prepare($conn, '
                UPDATE product SET 
                    ...
                    description = ?, category_id = ?, brand = ?, image = ?,
                    is_featured = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            ');
            mysqli_stmt_bind_param($stmt, 'sddisissiii',
                $name, $price, $sale_price, $stock, $description,
                $category_id, $brand, $image, $is_featured, $is_active, $id
            );
            mysqli_stmt_execute($stmt);
            ...
            $errors['db'] = 'Lỗi khi cập nhật sản phẩm: ' . $e->getMessage();
        }
    }
}
